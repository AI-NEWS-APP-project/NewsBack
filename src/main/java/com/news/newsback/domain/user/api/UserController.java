...
public class UserController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<CommonReponse<UserResponse>> signUp(
        ...
        return ResponseEntity
            .status(201)
            .body(CommonReponse.success(response));
    }

    @Operation(summary = "일반 로그인", description = "이메일과 비밀번호로 로그인하여 JWT 토큰을 발급받습니다.")
    @PostMapping("/login")
    public ResponseEntity<CommonReponse<AuthResponse>> login(
        ...
        return ResponseEntity.ok(CommonReponse.success(response));
    }

    @Operation(summary = "소셜 로그인", description = "카카오 또는 구글 액세스 토큰을 사용하여 로그인하거나 회원가입합니다.")
    @PostMapping("/social/{provider}")
    public ResponseEntity<CommonReponse<AuthResponse>> socialLogin(
    ...
    @PostMapping("/refresh")
    public ResponseEntity<CommonReponse<AuthResponse>> refresh(
    ...
    @PostMapping("/logout")
    public ResponseEntity<CommonReponse<Void>> logout(
            @Valid @RequestBody UserRequest.Logout request) {
        userService.logout(request.getRefreshToken());
        return ResponseEntity.ok(CommonReponse.success());
    }
